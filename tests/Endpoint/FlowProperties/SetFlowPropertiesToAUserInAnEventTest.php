...
class SetFlowPropertiesToAUserInAnEventTest extends EndpointTestCase
{
    /** @test **/
    public function call_set_properties_to_a_user_in_chat_for_event_endpoint()
    {
        ...
        $this->mockClient
            ->on(
                new RequestMatcher('/events/'.$eventId.'/users/'.$userId.'/properties'),
        ...
        $this->givenSdk()->flowProperties()->setForUserInAllChats(
            $eventId,
            $userId,
            $properties,
        );
